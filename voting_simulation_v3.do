...
// parties, 10 years
gen state = 1
replace state = 2 in 21/80
replace state = 3 in 81/`max_obs'

// Constant party numbers over time
gen num_parties_const = 2
replace num_parties_const = 6 if state == 2
replace num_parties_const = 12 if state == 3

// Party ID
gen party_id = 10
forvalues k = 20 30 to `max_obs' {
  local start_val = `k' - 9
  local end_val = `k'
...
// Adding number of parties
reg vote_share_scale incumbent x_indiv num_parties_const, vce(cluster state)
